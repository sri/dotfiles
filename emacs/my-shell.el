...
(defun my/shell-switch-to-next-most-recent (&optional buffer)
  ...
  (let ((shells (->> (buffer-list)
    ...
    (cond ((null shells)
           (shell))
          ((memq major-mode my/shell-modes)
...
(cl-defun my/open-shell-window-for-buffer (&optional open-cwd-p)
  ...
  (let ((current-dir (expand-file-name default-directory))
    ...
    (cl-labels ((buffer-dir (buffer)
      ...
      (cond ((my/shell-buffer-p (current-buffer))
             (cond ((window-in-direction 'above)
                    (delete-window))))
            (t
             (cond ((shell-buffer-below)
                    (windmove-down))
                   (t
                    (split-window-below)
                    (windmove-down)))
