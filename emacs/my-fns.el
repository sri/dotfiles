...
(defvar my/ediff-win-config)
(defvar my/ediff-restore-hook ()
  (when my/ediff-win-config
    (set-window-configuration my/ediff-win-config)
    (setq my/ediff-win-config nil)))

(require 'ediff)
(add-hook 'ediff-after-quit-hook-internal 'my/ediff-restore-hook)
...
(defun my/inspect-url (&optional url)
  ...
  (let* ((url (or url
                  ...
                  (read-string "URL: ")))
         (raw (string-trim url))
         (u (url-generic-parse-url raw))
         (path+query (or (url-filename u) ""))
         (parts (split-string path+query "\\?" t))
         (path (car parts))
         (query (cadr parts))
         (fragment (url-target u))
         (buf (get-buffer-create (generate-new-buffer "*URL Inspect*"))))
    (with-current-buffer buf
      (erase-buffer)
      (insert "Original:\n" raw "\n\n")
      (insert "Scheme:   " (or (url-type u) "") "\n")
      (insert "Host:     " (or (url-host u) "") "\n")
      (insert "Port:     " (if (url-portspec u)
                               (number-to-string (url-portspec u))
                             "")
              "\n")
      (insert "Path:     " (or path "") "\n")
      (insert "Fragment: " (or fragment "") "\n\n")
      (insert "Query params:\n")
      (if query
